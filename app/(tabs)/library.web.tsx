...
export default function LibraryScreen() {
  ...
  return (
    <View className="flex-1 bg-page-light dark:bg-page-dark">
      ...
      />
      {/* Choice Modal: Change or Remove */}
      <ConfirmModal
        visible={isEditChoiceVisible}
        title={t("library.exam_alert.title", "Manage Exam Date")}
        message={t(
          "library.exam_alert.message",
          "Would you like to change the date or remove it entirely?",
        )}
        confirmLabel={t("change_date")}
        cancelLabel={t("remove_date")}
        onConfirm={() => {
          setIsEditChoiceVisible(false);
          setWebPickerVisible(true);
        }}
        onCancel={() => {
          setIsEditChoiceVisible(false);
          saveLibraryExamDate(deckForExamDate.id, null); // Remove the date
        }}
        onClose={() => setIsEditChoiceVisible(false)}
      />
      ...
      >
        ...
        >
          <Pressable onPress={(e) => e.stopPropagation()}>
            <WebDatePicker
              selectedDate={tempExamDate}
              onSelect={(date: Date | null) => {
                // If date is null, we can treat it as "removing" the date
                // or just prevent saving if a date is required.
                setTempExamDate(date || new Date());
                if (date && deckForExamDate) {
